...
class DataTrainingArguments:
    ...
    use_combined_prompt_injection_datasets: bool = field(
        default=False,
        metadata={"help": "Whether to use the combined prompt injection datasets (jailbreak-classification, prompt-injections, prompt-injections-benchmark)."}
    )
    ...
    def __post_init__(self):
        if self.dataset_name is None and not self.use_combined_prompt_injection_datasets:
...
class ModelArguments:
    ...
    hidden_dropout_prob: Optional[float] = field(
        default=None,
        metadata={"help": "Dropout probability for hidden layers. If not specified, uses model default (typically 0.1)."},
    )
    attention_probs_dropout_prob: Optional[float] = field(
        default=None,
        metadata={"help": "Dropout probability for attention weights. If not specified, uses model default (typically 0.1)."},
    )
    use_deberta_xsmall: bool = field(
        default=False,
        metadata={"help": "Whether to use microsoft/deberta-v3-xsmall (True) or microsoft/deberta-v3-base (False)."},
    )
...
def main():
    ...
    # Override model selection if using deberta models
    if data_args.use_combined_prompt_injection_datasets:
        if model_args.use_deberta_xsmall:
            model_args.model_name_or_path = "microsoft/deberta-v3-xsmall"
            logger.info("Using microsoft/deberta-v3-xsmall model")
            # Recommended settings for xsmall model
            if training_args.learning_rate == 5e-5:  # If using default learning rate
                training_args.learning_rate = 2e-5
                logger.info("Setting learning rate to 2e-5 for xsmall model")
            if training_args.per_device_train_batch_size == 8:  # If using default
                training_args.per_device_train_batch_size = 16
                logger.info("Setting batch size to 16 for xsmall model")
            if training_args.num_train_epochs == 3.0:  # If using default
                training_args.num_train_epochs = 5.0
                logger.info("Setting epochs to 5 for xsmall model")
        else:
            model_args.model_name_or_path = "microsoft/deberta-v3-base"
            logger.info("Using microsoft/deberta-v3-base model")
            # Recommended settings for base model
            if training_args.learning_rate == 5e-5:  # If using default learning rate
                training_args.learning_rate = 1e-5
                logger.info("Setting learning rate to 1e-5 for base model")
            if training_args.per_device_train_batch_size == 8:  # If using default
                training_args.per_device_train_batch_size = 8
                logger.info("Setting batch size to 8 for base model")
            if training_args.num_train_epochs == 3.0:  # If using default
                training_args.num_train_epochs = 3.0
                logger.info("Setting epochs to 3 for base model")
    ...
    if data_args.use_combined_prompt_injection_datasets:
        logger.info("Loading and combining three prompt injection datasets...")
        
        # Load first dataset: jackhhao/jailbreak-classification
        # Has columns: prompt, type (jailbreak/benign)
        ...
        logger.info(f"Loaded jailbreak-classification: {dataset1}")
        # Rename columns to standardize: prompt -> text, type -> label
        dataset1 = dataset1.rename_column("prompt", "text")
        dataset1 = dataset1.rename_column("type", "label")
        
        # Load second dataset: deepset/prompt-injections
        # Has columns: text, label (injection/legitimate)
        ...
        logger.info(f"Loaded prompt-injections: {dataset2}")
        # This dataset already has text and label columns
        
        # Load third dataset: qualifire/prompt-injections-benchmark
        # Has columns: text, label (jailbreak/benign)
        ...
        logger.info(f"Loaded prompt-injections-benchmark: {dataset3}")
        # This dataset already has text and label columns
        
        # Standardize labels across all datasets
        # Map all to: 0 = benign/legitimate, 1 = jailbreak/injection
        def standardize_labels_dataset1(example):
            # jailbreak -> 1, benign -> 0
            if example["label"] == "jailbreak":
                example["label"] = 1
            else:  # benign
                example["label"] = 0
            return example
        
        def standardize_labels_dataset2(example):
            # injection -> 1, legitimate -> 0
        ...
        def standardize_labels_dataset3(example):
            # jailbreak -> 1, benign -> 0
            if example["label"] == "jailbreak":
                example["label"] = 1
            else:  # benign
        ...
        dataset1 = dataset1.map(standardize_labels_dataset1)
        dataset2 = dataset2.map(standardize_labels_dataset2)
        dataset3 = dataset3.map(standardize_labels_dataset3)
        
        # Combine train splits - handle different split names
        # dataset3 might have 'test' instead of 'train' split
        logger.info(f"Dataset3 available splits: {list(dataset3.keys())}")
        dataset3_split = dataset3["test"] if "test" in dataset3 else dataset3["train"]
        ...
        # Create validation split from 10% of combined data
        combined_data = combined_train.train_test_split(test_size=0.1, seed=training_args.seed)
        
        raw_datasets = datasets.DatasetDict({
            "train": combined_data["train"],
            "validation": combined_data["test"]
        })
        
        # Rename text column to sentence for consistency with the rest of the script
        raw_datasets = raw_datasets.rename_column("text", "sentence")
        
        logger.info(f"Final combined dataset: {raw_datasets}")
        logger.info(f"Train samples: {len(raw_datasets['train'])}, Validation samples: {len(raw_datasets['validation'])}")
        
        # Fix evaluation strategy since we now know validation data exists
        if training_args.do_eval and training_args.evaluation_strategy == "no":
            from transformers.trainer_utils import IntervalStrategy
            training_args.evaluation_strategy = IntervalStrategy.EPOCH
            logger.info("Set evaluation_strategy to EPOCH since validation data is available")
    ...
    if model_args.hidden_dropout_prob is not None:
        ...
        logger.info(f"Setting hidden_dropout_prob to {config.hidden_dropout_prob}")
    if model_args.attention_probs_dropout_prob is not None:
        config.attention_probs_dropout_prob = model_args.attention_probs_dropout_prob
        logger.info(f"Setting attention_probs_dropout_prob to {config.attention_probs_dropout_prob}")
    ...
    GPA.pc.append_module_names_to_track(["DebertaV2Embeddings", "DisentangledSelfAttention", "LayerNorm", "Embedding"])
    GPA.pc.append_module_names_to_convert(["DebertaV2SelfOutput", "DebertaV2Output"])
    GPA.pc.set_improvement_treshold([[0.01, 0.001, 0.0]])
    #GPA.pc.set_switch_mode(GPA.pc.DOING_SWITCH_EVERY_TIME)

    # Detect model's dtype and set PerforatedAI to match
    model_dtype = next(model.parameters()).dtype
    GPA.pc.set_d_type(model_dtype)
    print(f'Setting PerforatedAI dtype to match model: {model_dtype}')
    ...
    model.pooler.dense.set_this_output_dimensions([-1,0])
